    ...
    <div className="flex flex-col h-full bg-[#F1F5F9] text-slate-900 overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {[
            { label: 'Direction', score: result.overview.directionScore, sub: result.overview.overallVerdict, vtext: directionVerdictText },
            { label: 'Workflow / Execution', score: result.overview.executionScore, sub: getMatrixLabel(result.overview.matrixPosition), vtext: workflowVerdictText },
          ].map((c) => {
            ...
            return (
              <div key={c.label} className={`flex items-start gap-3 p-3 rounded-lg border ${st.ring} bg-slate-50/40`}>
                ...
                <div className="min-w-0">
                  <div className="text-[10px] uppercase font-mono text-slate-400 tracking-wider">{c.label}</div>
              ...
              <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 flex flex-col justify-between shadow-sm">
                <div>
                  ...
                  <p className="text-sm text-slate-600 mt-4 leading-relaxed bg-slate-50 p-4 rounded-lg border border-slate-200/80">
                    <span className="font-bold text-slate-800">Core Blocker:</span> {result.overview.mainDiagnosis}
                  </p>
                </div>
                        ...
                        <tr key={idx} className="hover:bg-slate-50/50">
                          ...
                          <td className="py-4 px-4 space-y-1 leading-normal">
                            ...
                            <p className="text-red-700 text-[10px] font-mono">
                              <span className="font-bold text-red-600">Risk:</span> {item.risk}
                            </p>
                          </td>
